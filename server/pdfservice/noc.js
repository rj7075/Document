...
export const generateNOCPDF = async (userData) => {
  ...
  const drawTextBlock = (text, size = fontSizeNormal, bold = false) => {
    ...
    lines.forEach((line) => {
      if (y < margin + size) {
        // force single-page NOC
        return;
      }
      page.drawText(line, { x: margin, y, size, font: bold ? fontBold : font });
      y -= size + lineGap;
    });
    y -= lineGap * 2;
  };
  ...
  // Header
  drawTextBlock("SARIN WORKSPACE LLP", fontSizeHeader, true);
  drawTextBlock("LLP Identification Number: ACP-8465", fontSizeNormal);
  drawTextBlock(
  ...
  const body = [
    ...
    `On expiry or termination, the company must take steps to remove/de-list the address from statutory records.`,
    ...
    "______________________________________________________________________Noida",
  ];

  body.forEach((line) => drawTextBlock(line, fontSizeNormal));
